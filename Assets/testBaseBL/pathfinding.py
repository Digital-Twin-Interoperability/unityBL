...
class RRTStar:
    def __init__(self, start, goal, num_obstacles, map_size, step_size=1.0, max_iter=500):
        ...
        self.obstacles = self.load_obstacles_from_isaac(obstacle_json)
    ...
    def load_obstacles_from_isaac(self, obstacle_json):
        """Load obstacles from Isaac Sim (formatted as list of raycast data)."""
        obstacles = []
        for ray in obstacle_json:
            hit_pos = ray["hit_position"]
            ox, oy = hit_pos[0], hit_pos[1]
            # Use the x and y position for obstacles
    ...
    def draw_obstacles(self):
        ...
        for (ox, oy) in self.obstacles:
            circle = plt.Circle((ox, oy), 0.5, color='r')  # Assuming radius is 0.5 for simplicity
    ...
    def steer(self, from_node, to_node):
        """Steer from one node to another, respecting robot kinematics or velocity."""
        theta = math.atan2(to_node.y - from_node.y, to_node.x - from_node.x)
        dx = self.step_size * math.cos(theta)
        dy = self.step_size * math.sin(theta)
    ...
    def is_collision_free(self, node):
        """Query raycasting data to check if node is in collision."""
        return self.omni_collision_check(node.x, node.y)

    def omni_collision_check(self, x, y):
        # Simple collision check by comparing the (x, y) to the obstacles
        for (ox, oy) in self.obstacles:
            if np.linalg.norm([x - ox, y - oy]) < 1.0:  # Assuming a threshold distance of 1 unit
...
if __name__ == "__main__":
    ...
    rrt_star = RRTStar(start, goal, map_size=map_size, step_size=1.0, max_iter=500)
    rrt_star.obstacles = rrt_star.load_obstacles_from_isaac(obstacle_json)
